...
echo "$($PSQL "SELECT name FROM teams, games WHERE year = 2014 AND round = 'Eighth-Final' AND (team_id = winner_id OR team_id = opponent_id)
 ORDER BY name")"
...
echo "$($PSQL "SELECT year, name FROM teams, games WHERE round = 'Final' AND team_id = winner_id ORDER BY year")"

echo -e "\nList of teams that start with 'Co':"
echo "$($PSQL "SELECT DISTINCT(name) FROM teams, games WHERE (team_id = winner_id or team_id=opponent_id) AND name LIKE 'Co%'")"
